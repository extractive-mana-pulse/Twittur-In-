...
- **Next task:** keep replicating the slice pattern feature-by-feature (Section 8 is the recipe). Remaining: detail (tweet replies), follow, chat/settings/feedback/connection (Section 10), plus the deferred **profile image picker** (expect/actual) and the deferred **tweet like/edit/replies/report** (Section 10 tweet row). Home shows a **Feed** entry now; folding the feed into Home as its literal body (drawer/FAB) is still open.
...
:feature:tweet:domain :feature:tweet:data :feature:tweet:presentation                         ← tweet feed/compose/delete (like/edit/replies/report deferred)
```
