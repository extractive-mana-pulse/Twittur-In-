...
import com.example.twitturin.search.presentation.screens.SearchScreen
...
//
//            searchViewModel.searchUser.observe(viewLifecycleOwner) { response ->
//                when (response) {
...
//        }
    ...
    private fun searchClickEvent(
        searchCLickEvents : SearchAdapter.SearchCLickEvents,
        ...
        when (searchCLickEvents) {
            ...
            SearchAdapter.SearchCLickEvents.FOLLOW -> {
                ...
                followViewModel.follow.observe(viewLifecycleOwner) {
                    when (it) {
                        is Follow.Success -> {
                            binding.searchRootLayout.snackbar(
                                binding.searchRootLayout,
                                searchUser.username.toString()
                            )
                        }
