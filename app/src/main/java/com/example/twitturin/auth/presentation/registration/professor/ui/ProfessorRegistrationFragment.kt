                        ...
                        is ProfRegUiEvent.OnAuthPressed -> {
                            ...
                            repeatOnStarted {
                                signUpViewModel.profRegResult.collectLatest { result ->
                                    when (result) {
                                        is SignUpProfResult.Success -> { findNavController().navigate(R.id.action_professorRegistrationFragment_to_signInFragment) }
                                        is SignUpProfResult.Error -> { profRegRootLayout.snackbarError(signUpProf, result.message, ""){} }
                                        SignUpProfResult.Loading -> profRegRootLayout.snackbar(signUpProf,R.string.loading.toString())
                                    }
                                }
                            }
                        }
